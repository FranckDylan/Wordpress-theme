<?php
function wpc_cpt_clients() {

	/* clients */
	$labels = array(
		'name'                => _x('Clients', 'Post Type General Name', 'textdomain'),
		'singular_name'       => _x('client', 'Post Type Singular Name', 'textdomain'),
		'menu_name'           => __('Clients', 'technum-theme'),
		'name_admin_bar'      => __('Clients', 'technum-theme'),
		'parent_item_colon'   => __('Parent Client:', 'technum-theme'),
		'all_items'           => __('All Clients', 'technum-theme'),
		'add_new_item'        => __('Add New Client', 'technum-theme'),
		'add_new'             => __('Add Client', 'technum-theme'),
		'new_item'            => __('New Client', 'technum-theme' ),
		'edit_item'           => __('Edit Client', 'technum-theme'),
		'update_item'         => __('Update Client', 'technum-theme'),
		'view_item'           => __('View Client', 'technum-theme'),
		'search_items'        => __('Search Client', 'technum-theme'),
		'not_found'           => __('Not found', 'technum-theme'),
		'not_found_in_trash'  => __('Not found in Trash', 'technum-theme'),
	);
	$rewrite = array(
		'slug'                => _x('clients', 'clients', 'textdomain'),
		'with_front'          => true,
		'pages'               => true,
		'feeds'               => false,
	);
	$args = array(
		'label'               => __('client', 'technum-theme'),
		'description'         => __('client', 'technum-theme'),
		'labels'              => $labels,
		'supports'            => array('title', 'editor', 'revisions','page-attributes', 'thumbnail'),
		'taxonomies'          => array('clients_type'),
		'hierarchical'        => true,
		'public'              => true,
		'show_ui'             => true,
		'show_in_menu'        => true,
...
add_action('init', 'wpc_cpt_clients', 10);

//Ajout dans les résultats de recherche
function wpc_cpt_in_search_clients($query) {
...
add_action('pre_get_posts','wpc_cpt_in_search_clients');

//Ajout d'une meta box pour le sticky post
add_action( 'add_meta_boxes', 'meta_box_add_clients' );
add_action( 'save_post', 'pseudosticky_save_client',10,2);
add_action('save_post', 'description_save_client', 10, 2);


function meta_box_add_clients()
{
  add_meta_box('pseudosticky', __('Mettre en avant sur la page d\'acceuil', 'technum-theme'), 'addbox_clients', 'clients', 'side', 'high');
  add_meta_box(
	'description', // Metabox ID
	__('Brève Description', 'technum-theme'), // Title to display
	'add_new_box_clients', // Function to call that contains the metabox content
	'clients', // Post type to display metabox on
...
function add_new_box_clients($post, $metabox) {  
	// Use nonce for verification
	
	  wp_nonce_field(basename( __FILE__ ), 'client_description_nonce' );
	  $description = get_post_meta( $post->ID, 'description', true ); // Get the saved values
	  $url = get_post_meta( $post->ID, 'url_client', true ); // Get the saved values
		...
		<div>
			...
			<input type="url" id="url" name="url_client" rows="6" style="width:100%;" value="<?php echo esc_attr( $url ); ?>" >
		</div>
...
function addbox_clients($post, $metabox) {  
// Use nonce for verification
...
function description_save_client( $post_id , $post) {
	
	if ($post->post_type != 'clients'){
		return;
	}
  // Verify the nonce before proceeding.
    if ( !isset( $_POST['client_description_nonce'] ) || !wp_verify_nonce( $_POST['client_description_nonce'], basename( __FILE__ ) ) )
        return $post_id;
    ...
    if ( !current_user_can( $post_type->cap->edit_post, $post_id ) )
		...
		$new_meta_value ="";
    	$meta_key="";
		$champs = array('description', 'url_client');
...
function pseudosticky_save_client( $post_id , $post) {
 ...
 */
function itgstore_page_clients_liste(){
...
function itgstore_front_actualite_client_section($slug){
...
function itgstore_get_reussites_clients($reussiteId){

	?>
	<div class="client shadow p-5 ml-3">
		<div  class="container mb-4 pl-0 ml-0">
			<h2 style="color: #042F6B; margin-left: auto;
			margin-right: auto;" class="title"><?php function_exists('pll_e') ? pll_e('client') : _e('client', 'textdomain'); ?></h2>
		</div>
		<?php  ?>
		<div style="width:100%; ">
			<img style="width:150px; height: 150px;" src="<?php echo get_the_post_thumbnail_url(get_post_meta($reussiteId, 'client_id', true)) ?>" />
		</div>
		<?php ?>
	</div>
	<?php
